const CAPABILITIES = ['config:read', 'plc:read', 'prometheus:read', 'mongodb:read', 'mongodb:write', 'usb:read', 'usb:write', 'display:present', 'network:outbound', 'hotspot:manage'];
function validateManifest(app, { release = false } = {}) {
  ...
  if (!['1', '1.1', '1.2'].includes(sdk.sdkVersion)) errors.push('metadata.orenda.sdkVersion must be "1", "1.1" or "1.2"');
  ...
  if (Array.isArray(sdk.capabilities) && sdk.capabilities.includes('hotspot:manage') && sdk.sdkVersion !== '1.2') errors.push('hotspot:manage requires sdkVersion "1.2"');
  if (release) {
